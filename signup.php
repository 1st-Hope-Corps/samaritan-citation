...
if(isset($_POST['op'])){
	...
	if ($pass != $c_pass) {
		$_SESSION['validation_error'] = 'Password and Confirm password does not match.';
	...
	$registeredUserData = db_fetch_object(db_query("SELECT * FROM {users} WHERE name = '%s' OR mail = '%s'", $registration_name, $email));

	if (!$registeredUserData) {
		$user = user_save(NULL, $user_data);
		sendCustomMailer($user->mail, 'You\'ve successfully signup to Samaritan Citation', "Hi ". $registration_name .",
			<br /><br />
			You successfully created an account to Samaritan Citation.");
	...
	$('.role_signup').change(function(){
		...
		switch (val) {
			...
			case '2':
				...
				$('.reporter-fields').find('input').attr('required', true);
				$('.reviewer-fields').hide();
				$('.reviewer-fields').find('input').removeAttr('required');
				break;
			case '3':				
				$('.reporter-fields').hide();
				$('.reviewer-fields').show();

				$('.reporter-fields').find('input').removeAttr('required');
				$('.reviewer-fields').find('input').attr('required', true);
				break
		}
	});
});
