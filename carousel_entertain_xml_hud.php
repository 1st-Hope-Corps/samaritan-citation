...
case "links":
	...
	while ($oLinks = db_fetch_object($oLinksResult)){
		...
		$sOutput .= '<item>
						...
						<link>http://'.$_SERVER["HTTP_HOST"].base_path().'mystudies/content/url/wrapper/'.$oLinks->id.'/'.$sSiteType.'/'.base64_encode($oLinks->url).'</link>
	...
	if ($sOutput == ""){
		$sOutput = '<item>
						...
						<description><![CDATA[1No content to display, yet.]]></description>
						<group_id>0</group_id>
						<link>http://www.firstearthalliance.org/hud.php</link>
						<target>_blank</target>
					</item>';
	}
...
case "tags":
	...
	mysqli_free_result($oSitesResult);
	...
	for ($i=1; $i<count($aSitesGroup); $i++){
		...
		if ($iRawGroupLevel == $iGroupLevel || $sTypeSub == 1){
			...
			mysqli_free_result($oTagsResult);
		...
		mysqli_free_result($iRawGroupLevel);
	}
...
case "files":
	...
	while ($oFiles = db_fetch_object($oFilesResult)){
		...
		switch ($sTypeSub){
			case "image":
				if ($sFileType == $sTypeSub){
                    ...
                    $sFileFulldiv = $sBasePath."mystudies/file/image/".$iGroupLevel."/view/".$sId;
                    $sFileFull = 'http://'.$_SERVER["HTTP_HOST"].base_path().'mystudies/content/url/wrapper/'.$sId.'/'.$sFileType.'/'.base64_encode($sFileFulldiv);
				}elseif ($sFileType == "image_embed"){
					$sRawURL = GetAttribute("src", $oFiles->sEmbedCode);
					$sFileURL = $sRawURL;
					//$sFileFull = 'http://'.$_SERVER["HTTP_HOST"].base_path().'mystudies/content/url/wrapper/'.$sId.'/'.$sFileType.'/'.base64_encode($sRawURL);
					$sFileFulldiv = $sBasePath."mystudies/file/image/".$iGroupLevel."/view/".$sId;
                    $sFileFull = 'http://'.$_SERVER["HTTP_HOST"].base_path().'mystudies/content/url/wrapper/'.$sId.'/'.$sFileType.'/'.base64_encode($sFileFulldiv);
                }else{
					$sFileURL = $sImageURL.$oFiles->sEmbedCode;
					$sFileFull = 'http://'.$_SERVER["HTTP_HOST"].base_path().'mystudies/content/url/wrapper/'.$sId.'/'.$sFileType.'/'.base64_encode($oFiles->sEmbedCode);
			...
			case "video":
				// firstearthalliance.org/carousel_xml_hud.php?i=18&q=files&qq=video&qqq=rec
		...
		if (!empty($oFiles->sEmbedCode) && strpos($oFiles->sEmbedCode, "google.com/a/hopecybrary.org/") !== false && ($oFiles->sFileType == "doc_ext" || $oFiles->sFileType == "image_ext")){
			if (!empty($oFiles->sFileId))
				...
				$sFileURL = str_replace("&stwurl=","&stwdelay=5&stwurl=",$sImageURL) .'http://firstearthalliance.org/viewer.php?pg=' . $oFiles->id; 	
			...
			if (preg_match("/src=\"?([^\>|\"|\s]+)/s",$oFiles->sEmbedCode,$match)) {
				$sFileURL = str_replace("&stwurl=","&stwdelay=5&stwurl=",$sImageURL) .'http://firstearthalliance.org/viewer.php?pg=' . $oFiles->id; 
				$sFileType = "doc_ext";
				$sFileFull = 'http://'.$_SERVER["HTTP_HOST"].base_path().'mystudies/content/url/wrapper/'.$sId.'/'.$sFileType.'/'.base64_encode($match[1]);		
			}
		}
	...
	if ($sOutput == ""){
		$sOutput = '<item>
						...
						<name><![CDATA[No Content]]></name>
						<thumb><![CDATA['.$sBasePath.'hud_files/images/carousel_no_data.png]]></thumb>
						<description><![CDATA[2No content to display, yet.]]></description>
						<group_id>0</group_id>
						<link>http://www.firstearthalliance.org/hud.php</link>
						<target>_blank</target>
					</item>';
...
case "contents":
	...
	break;
	case 'other':
		$sWhereClause .= " AND sFileGroup = 'other'";
	break;
	case '':
	case 'rec':
		$sWhereClause .= " AND sFileGroup = 'rec'";
	}
	...
	/*
	$restessd = $sqlFiles.' '.$sTypeSub."_ext".' '.$sTypeSub."_embed".' '.$sTypeSub."_youtube".' '.$sOption;
	$oFilesResult = db_query($sqlFiles, array($sTypeSub, $sTypeSub."_ext", $sTypeSub."_embed", $sTypeSub."_youtube", $sOption));
	*/
	$restessd = $sqlFiles.' '.$sTypeSub."_ext".' '.$sTypeSub."_embed".' '.$sTypeSub.'_youtube '.$sOption;
	$oFilesResult = db_query($sqlFiles, array($sTypeSub, $sTypeSub."_ext", $sTypeSub."_embed", "video_youtube", $sOption));
	...
	while ($oFiles = db_fetch_object($oFilesResult)){
		...
		$sFileId = $oFiles->sFileId;
		$sFileTitle = $oFiles->sTitle;
		$sFileEmbed = $oFiles->sEmbedCode;
		$sFileDesc = mb_convert_encoding($oFiles->sDesc, "UTF-8", "UTF-8");
		
		switch ($sTypeSub){
			case "content":
				if ($sFileType == "content_embed"){
					$sRawURL = GetAttribute("src", $oFiles->sEmbedCode);
					//$sFileURL = $sRawURL;
					$sFileFulldiv = $sBasePath."entertainment/file/image/".$iGroupLevel."/view/".$sId;
					$sFileFull = 'http://'.$_SERVER["HTTP_HOST"].base_path().'entertainment/content/url/wrapper/'.$sId.'/'.$sFileType.'/'.base64_encode($sFileFulldiv);
					$sFileType = 'link';
				} else if($sFileType == "content_ext"){
					$sFileURL = $sImageURL.$oFiles->sEmbedCode;
					$sFileFull = 'http://'.$_SERVER["HTTP_HOST"].base_path().'entertainment/content/url/wrapper/'.$sId.'/'.$sFileType.'/'.base64_encode($oFiles->sEmbedCode);
					$sFileType = 'link';
				} else if($sFileType == "video_youtube"){
				//start		
				/*		
					if(strpos($oFiles->sEmbedCode, 'iframe')){     
						preg_match("/youtube\.com\/embed\/([0-9a-zA-Z_-]+)/s",$oFiles->sEmbedCode,$aiframe);
						...
						$code1 = explode('=',$oFiles->sEmbedCode);
						if(strpos($code1[1],"feature")){
						$code2 = explode('&',$code1[1]);
						$code = $code2[0];
						} else{
						$code = $code1[1];
						}
						$sFileURL = "http://i.ytimg.com/vi/" . $code . "/hqdefault.jpg";	
						$sFileFull = 'http://www.youtube.com/v/'.$code.'&hl=en&fs=1&rel=0&color1=0x234900&color2=0x4e9e00&border=1';
						$sFileType = "link";
					}
			    */
		...
		case 'content_embed':
			/*if(strpos($oFiles->sEmbedCode, 'iframe')){
				...
				//$sFileURL = $oEmbedMovie->getAttribute("src");
				//$sFileURL = 'http://www.firsthopecorps.org/viewergames.php?reload=1';
				$sFileURL = $sFileId;
			
			} elseif(strpos(strtolower($oFiles->sEmbedCode), 'object')){
				$oDomDoc = new DOMDocument();   
				@$oDomDoc->loadHTML($oFiles->sEmbedCode); 
			...
			$sFileURL = '';
			$sFileURL = 'http://images.shrinktheweb.com/xino.php?stwembed=1&stwu=1aaf6&stwaccesskeyid=85755ac2242b24a&stwxmax=2020&stwymax=1284&stwinside=1&stwurl=http://www.hopecybrary.org/viewergames.php?pg='.$oFiles->id;
		break;
		}
		//$sFileURL = 'http://www.dailyfreegames.com/images/files/jungleplunge.swf';
		
		if(strpos($oFiles->sEmbedCode, 'spoon') !== false){
		$sFileType = "";
		$sFileURL = $sFileId;
		}
...
case "tagall":
	...
	//$oSiteFaveResult = db_query($sqlFave, $aOption3);
	...
	mysqli_free_result($oSiteRecResult);
	...
	while ($oDoc = db_fetch_object($oDocResult)){
		...
		if ($sFileType == "doc"){
			$sFileURL = $sBasePath.'misc/file_doc.png';
			$sFileFull = $sBasePath."mystudies/file/doc/".$iGroupLevel."/view/".$oDoc->id;
	...
	mysqli_free_result($oAnimeResult);
...
case "leaf":
	...
	mysqli_free_result($iSiteFaveCount);
	mysqli_free_result($iSiteRecCount);
	mysqli_free_result($iSiteOtherCount);
	// --END Data Count
	...
	$sGamesIcon = ($sTypeSub == "hud") ? $sImageDir."featured_f7.png":"";
/*	
	$sWebSite = '<item>
					<uid>1</uid>
					<name><![CDATA[Web Sites ('.$iWebSiteCount.')]]></name>
	...
	$sGames = '<item>
					<uid>7</uid>
					<name><![CDATA[Games]]></name>
					<thumb>'.$sGamesIcon.'</thumb>
					<description><![CDATA[Games Description]]></description>
					<group_id>'.$iGroupLevel.'</group_id>
					<link>empty.html</link>
				</item>';
	*/
	...
	mysqli_free_result($iPhotoCount);
	mysqli_free_result($iDocCount);
	mysqli_free_result($iAnimeCount);
