<?php
/**
 * Different settings loaded depending on the environment
 *
 * */

# First check for a local settings file, used in local dev environments
# This local settings file will be ignored by version control (in .gitignore)
if ( file_exists( dirname( __FILE__ ) . '/local.settings.php' ) ) {
	include( dirname( __FILE__ ) . '/local.settings.php' );
} else {

	# Define the hostnames of dev servers
	$aDevServers = array( 'dev.hopecybrary.org', 'hopedev.jelastic.tsukaeru.net' );

	# Use the provided URL to determine the environment
	if ( in_array( $_SERVER['SERVER_NAME'], $aDevServers ) ) {
		# Developmenet server
		include( dirname( __FILE__ ) . '/dev.settings.php' );
	} else {
		# Live server
		include( dirname( __FILE__ ) . '/live.settings.php' );
	}
