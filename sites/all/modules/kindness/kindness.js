...
$(document).ready(
	function(){
		...
		$("#kindness_VolunteerDeactivateDialog").dialog(
			{
				...
				buttons: {
					"Deactivate Account": function(){
						location = Drupal.settings.basePath+"kindness/mentor/deactivate";
					},
					"Cancel": function(){
						$(this).dialog("close");
					}
		...
		$('#delete_kindness').click(function(){
			let workz_to_delete = [];

			$("#id_uid:checked").each(
				function(){
					workz_to_delete.push($(this).val());
				}
			);

			if (workz_to_delete.length == 0 ) {
				alert('Please select atleast 1 citation to delete');
				return;
			}

			if (confirm("Are you sure you want to delete selected citation?") === true) {
			  	$.ajax({
			        url: "/kindness/callback/delete",
			        type: 'POST',
			        data: {
			        	'workz_to_delete': workz_to_delete
			        } ,
		...
		$("button[name^="+sPreButtons+"]").each(
			function(){
				if($("button#"+this.id).attr('hasClick')){
					return;
				}
				$("button#"+this.id).click(
					function(){
						var sButtonType = $(this).attr('value');
						...
						if ($("input[name='id_uid']:checked").length > 0){
							...
							$("#bApprove").val(((sButtonType == "approve") ? '1':'0'));
							$("#bDelete").val(sButtonType);
							$("#kindness_mentor_comment_type").text(sCommentText);
							$("#kindness_mentor_comment").center().show("slow");
						}else{
							alert(' Please select atleast 1 citation to '+sButtonType+'.');
						}
					}
				);
				
				$("button#"+this.id).attr('hasClick', true)
			}
		);
			...
			function(){
				...
				$("img#"+sThisId).click(
					function(){
						...
						$.post(
							Drupal.settings.basePath+"kindness/callback/comments",
							{
