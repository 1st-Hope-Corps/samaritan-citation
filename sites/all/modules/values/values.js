...
$(document).ready(
	function(){
		...
		var aThumbView = Array('values_site_thumb_', 'values_image_thumb_');
		...
		$("#values_edit_mode").click(
			function(){
				$("#values_edit_mode_notice").center().toggle("slow");
			}
		);
		
		$("#values_edit_mode_notice_cancel").click(
			function(){
				$("#values_edit_mode_notice").toggle("slow");
			}
		);
		
		$("#values_edit_mode_url_notice_cancel").click(
			function(){
				$("#values_edit_mode_url_notice").toggle("slow");
			}
		);
		
		$("#values_edit_mode_notice_amend").click(
			function(){
				$("#values_edit_mode_notice").toggle("slow");
				location = values_sBasePath+"values/edit/"+values_iGroupLevel;
			}
		);
		
		$("#values_edit_mode_notice_add").click(
			function(){
				$("#values_edit_mode_notice").toggle("slow");
				location = values_sBasePath+"values/edit/"+values_iGroupLevel+"/add";
			}
		);
		
		$("#values_edit_mode_url").click(
			function(){
				$("#values_edit_mode_url_notice").center().toggle("slow");
			}
		);
		
		$("#values_edit_mode_url_notice_amend").click(
			function(){
				$("#values_edit_mode_url_notice").toggle("slow");
				location = values_sBasePath+"values/url/edit/"+values_iGroupLevel;
			}
		);
		
		$("#values_edit_mode_url_notice_add").click(
			function(){
				$("#values_edit_mode_url_notice").toggle("slow");
				location = values_sBasePath+"values/url/edit/"+values_iGroupLevel+"/add";
			}
		);
		
		$("#values_edit_mode_url_animation").click(
			function(){
				$("#values_edit_mode_url_animation_notice").center().toggle("slow");
			}
		);
		
		$("#values_edit_mode_url_animation_notice_amend").click(
			function(){
				$("#values_edit_mode_url_animation_notice").toggle("slow");
				location = values_sBasePath+"values/url/animation/edit/"+values_iGroupLevel;
			}
		);
		
		$("#valuesvalues_edit_mode_url_animation_notice_add").click(
			function(){
				$("#values_edit_mode_url_animation_notice").toggle("slow");
				location = values_sBasePath+"values/url/animation/edit/"+values_iGroupLevel+"/add";
			}
		);
		
		$("#values_edit_mode_url_animation_notice_cancel").click(
			function(){
				$("#values_edit_mode_url_animation_notice").toggle("slow");
			}
		);
		...
		$("a[id^=values_file_tooltip_]").tooltip();
		
		
		var sLinkRecPre = "values_site_rec_link_";
		var sLinkOtherPre = "values_site_other_link_";
			...
			function(){
				$("a#"+this.id).click(
					function(){
						...
						$.post(
							values_sBasePath+"values/url/click",
							{
								sType: "site_rec",
								iRefId: iSiteId
							},
			...
			function(){
				...
				$("button#" + sButtonRecPre + iRecId).click(
					function(){
						...
						$("#sType").val("site_rec");
						$("#iRecId").val(iRecId);
						$("#values_DeleteComment").center().show("slow");
					}
				);
			}
		);
		...
		$("#btnDeleteCancel").click(
			function(){
				$("#values_DeleteComment").hide("slow");
			}
		);
		...
		$("#values_back_button").click(
			function(){
				history.back(1);
			}
		);
		
		$("#values_edit_mode_file_notice_amend").click(
			function(){
				$("#values_edit_mode_notice").toggle("slow");
				location = values_sBasePath+"values/file/"+values_sFileType+"/"+values_iGroupLevel+"/edit";
			}
		);
		
		$("#values_edit_mode_file_notice_add").click(
			function(){
				$("#values_edit_mode_notice").toggle("slow");
				location = values_sBasePath+"values/file/"+values_sFileType+"/"+values_iGroupLevel+"/edit/add";
			}
		);
		...
		$("div[id^=changelog_partial_]").each(
			function(){
				...
				$(this)
					...
					.click(
						function(){
							var sTitleVal = ($("#editors_sTitle_"+aLogTypeId[1]).length == 1) ? $("#editors_sTitle_"+aLogTypeId[1]).val():$("#values_sTitle").val();
		...
		if(true || jQuery.cookie && jQuery.cookie('glang') == null){
			if ($("#values_VolunteerCatList").length == 1){
				$("#values_VolunteerCatList").treeview(
					{url: Drupal.settings.basePath+"askeet/question/cats/values", unique: true}
				);
			}
			
			if ($("#values_VolunteerCatList2").length == 1){
				$("#values_VolunteerCatList2").treeview(
					{url: Drupal.settings.basePath+"askeet/question/cats/values", unique: true}
				);
			}
			
			if ($("#valuesadmin_VolunteerCatList").length == 1){
				$("#valuesadmin_VolunteerCatList").treeview(
					{url: Drupal.settings.basePath+"askeet/question/cats/valuesadmin", unique: true}
				);
			}
		}
			...
			.click(
				function(){
					if ($("span#guide_editor_value").text() != "none"){
						...
						$('#values_NoFeatureDialog').dialog('open');
					}
				}
			);
		
		if ($('#values_NoFeatureDialog').length == 1){
			$('#values_NoFeatureDialog').dialog(
				{
					autoOpen: false,
					resizable: false,
					modal: true,
		...
		$("#values_editors_full_cat")
			...
			.click(
				function(){
					$("#values_editors_cats").show();
				}
			);
		
		$("#values_editors_cat_cancel").click(
			function(){
				$("#values_editors_cats").hide();
			}
		);
		
		$("#values_editors_cat_set").click(
			function(){
				$("#values_editors_cats").hide();
			}
		);
		
		$("#values_guides_block_show").click(
			function(){
				$("#values_guides_block").css("top", ($(this).offset().top-430)+"px").toggle();
			}
		);
		...
		$("#editors_btnGo").click(
			function(){
				...
				if (sSelectedType == "add"){
					$("form#values_add input#volunteer_iGroupId").val(iVolunteerCatId);
			...
			function(){
				$(this).click(
					function(){
						...
						$("#values_ExistingCatDeleteComment").center().show("slow");
					}
				);
			}
		);
		...
		$("#btnExistingCatDeleteCancel").click(
			function(){
				$("#values_ExistingCatDeleteComment").hide("slow");
			}
		);
			...
			function(){
				...
				$(this).click(
					function(){
						...
						$("#sType").val(sSubType);
						$("#iRecId").val(iContentId);
						$("#values_ExistingItemDeleteComment").center().show("slow");
					}
				);
			}
		);
		...
		$("#btnExistingItemDeleteCancel").click(
			function(){
				$("#values_ExistingItemDeleteComment").hide("slow");
			}
		);
				...
				$(this)
					.click(
						function(){
							...
							$("#values_editors_cats").css("top", iPageY - 420).show();
						}
					)
					.hover(
						function(){
		...
		$("#values_editors_cat_change").click(
			function (){
				var iContentId = $("#volunteer_iContentId").val();
				var iGroupId = $("#volunteer_iGroupId_edit").val();
				
				$("#editors_iGroupId_"+iContentId).val(iGroupId);
				$("#editors_change_cat_"+iContentId).load(Drupal.settings.basePath+"values/getinvolved/full/cat/"+iGroupId);
				
				$("#values_editors_cats").hide();
			}
		);
		
		$("#values_btnVolunteerDeactivate")
			.hover(
				function(){
					$(this).css({"cursor":"pointer"});
				},
			...
			.click(
				function(){
					$("#values_VolunteerDeactivateDialog").dialog("open");
				}
			);
			
		$("#values_VolunteerDeactivateDialog").dialog(
			{
				modal: true,
				autoOpen: false,
				resizable: false,
				...
				buttons: {
					"Deactivate Account": function(){
						location = Drupal.settings.basePath+"values/getinvolved/volunteer/"+$("#values_sVolunteerType").val()+"/deactivate";
					},
					"Cancel": function(){
						$(this).dialog("close");
					}
		...
		$('form[id=pendingForm] button[name=btnApprove]').click(
			function(){
				var iCheckedBoxLen = $('form[id=pendingForm] input[type=checkbox]:checked').length;
				var oCheckedBox = $('form[id=pendingForm] input[type=checkbox]');
				var bProcess = true;
				...
				oCheckedBox.each(
					function(){
						if ($(this).is(':checked')){
							...
							if ($("div#rating_ShowRating_"+iItemId).length == 1 
								&& $("div#rating_ShowRating_"+iItemId).text() != 'Your rating has been posted.'
								&& $("div#rating_ShowRating_"+iItemId).text() != "You already rated this content."){
								
								$("div#rating_ShowRating_"+iItemId).addClass('notice');
									
									if (bProcess) bProcess = false;
							}else{
								$("div#rating_ShowRating_"+iItemId).removeClass('notice');
							}
						}
					}
				);
				...
				if (iCheckedBoxLen <= 0){
					alert('Please select, at least, one item/content to approve/disapprove.');
...
function searchContent(page){
	...
	$.ajax({
		...
		url: Drupal.settings.basePath+"values/getinvolved/search/contents/"+page,
		dataType:	'html',
		timeout:	100000000,
		cache: false,
...
function ValidateUpload(){
	...
	if ($("#sFileType").val() == "video")
		document.getElementById("values_file_add").action = document.getElementById("response_url").value;
...
function ValidateSubjDel(){
	...
	$("input[id^=values_del_subj_]").each(
		function(){
	...
	if (iDelSubjCount == 1){
		var bConfirm = confirm("Are you sure you want to delete this Subject/Category and all data under it?\n\nBe careful. This action cannot be undone.");
		
		return bConfirm;
	}else if (iDelSubjCount == 0){
		alert("Please select a Subject/Category to delete first.");
	}else{
		alert("Please select ONLY 1 Subject/Category to delete.");
	}
...
function ResizeImage(){
	$("#values_file_content img.thumbnail").each(
		function(){
			var iMaxWidth = 100;
			var iMaxHeight = 100;
			...
			if ($(this).attr("src") != values_sBasePath+"misc/file_doc.png"){
...
function checkEditorRating(jform,usertype) {
	...
	if (disapproved == 'No'){
		...
		if (usertype != "editors") return true;
...
function Values_getCatID(idCat, idLevel, eTitle){
	if (idCat !== ''){
		$("#volunteer_iGroupId").val(idCat);
		$("#editors_cat_form_sGroupLevel").val(eTitle); 
